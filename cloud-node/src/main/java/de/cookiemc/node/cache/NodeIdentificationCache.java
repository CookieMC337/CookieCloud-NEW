...
@Getter @Setter
public class NodeIdentificationCache implements IdentificationCache {
    ...
    public NodeIdentificationCache() {
        this.cachedUniqueIds = new ConcurrentHashMap<>();

        this.cacheFile = new File(NodeDriver.STORAGE_FOLDER, "cache.json");

        NodeDriver.getInstance().getNetworkExecutor().registerPacketHandler((PacketHandler<CachedUUIDPacket>) (wrapper, packet) -> {
