...
import de.cookiemc.http.api.HttpMessage;
import de.cookiemc.http.api.HttpVersion;
...
public interface NettyDefaultHttpMessage<M extends HttpMessage<?>> extends HttpMessage<M> {
	...
	default M setBody(@Nonnull Document document) {
		return setBody(document.asRawJsonString());
	}

	@Nonnull
	@Override
	default M setBody(@Nonnull Collection<Document> array) {
		return setBody(DocumentFactory.newJsonBundle(array).asRawJsonString());
	}
