...
import de.cookiemc.common.collection.ClassWalker;
...
import de.cookiemc.driver.event.*;
...
import de.cookiemc.driver.networking.protocol.packets.defaults.DriverCallEventPacket;
import de.cookiemc.driver.event.*;
...
public class DefaultEventManager implements IEventManager {
	...
	public IEventManager registerListener(@Nonnull Object listener) {
		for (Method method : ReflectionUtils.getMethodsAnnotatedWith(listener.getClass(), de.cookiemc.driver.event.EventListener.class)) {
			...
			de.cookiemc.driver.event.EventListener annotation = method.getAnnotation(EventListener.class);
