...
import de.cookiemc.driver.commands.data.CommandFlag;
import de.cookiemc.driver.commands.exceptions.CommandException;
import de.cookiemc.common.util.Operator;
 ...
 * Some child classes are {@link CommandArguments} or {@link CommandFlag} which uses this
 * class to nicely fetch values from the parameters
 */
public abstract class AbstractBundledParameters {
    ...
    protected AbstractBundledParameters(List<String> parameter) {
        this.rawParameter = parameter;
        this.parameter = parameter;
    }

    protected AbstractBundledParameters(String commandLine) {
     ...
     * @return This
     */
    public static AbstractBundledParameters newInstance(List<String> l) {
        return new AbstractBundledParameters(l) {
        };
    }

    public static AbstractBundledParameters newInstance(String commandLine) {
        return new AbstractBundledParameters(commandLine) {
        };
    }
    ...
    public static List<String> retrieveArguments(String commandLine, boolean mindEmpty) {
        List<String> l = StringUtils.getStringList(StringUtils.find(PARAMETER_PATTERN.pattern(), commandLine),
                s -> s.replaceAll("\"", ""));
    ...
    public boolean isEmpty() {
        return size() == 0 || (getParameter().stream().allMatch(s -> s.trim().isEmpty()));
    }
    ...
    public <T> T get(int paramIndex, Class<T> tClass, T backupVal) {
        ...
        CommandParameterType<T> type = CloudDriver.getInstance().getProviderRegistry().getUnchecked(ICommandManager.class).getParamTypeRegistry().get(tClass);
        if (type == null) {
            return backupVal;
        }
