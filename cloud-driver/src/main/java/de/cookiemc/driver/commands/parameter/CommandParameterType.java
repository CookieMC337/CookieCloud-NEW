...
import de.cookiemc.common.util.NumberUtil;
 ...
 * To fetch a custom param type you have to use {@link AbstractBundledParameters#get(int, Class)}
 *
 * @param <E> The type
 */
public abstract class CommandParameterType<E> {
    ...
    public <T> boolean checkRange(String arg, T t1, T t2) {
        return Validation.NUMBER.matches(arg)
                && NumberUtil.checkRange(ReflectionUtils.safeCast(arg), t1, t2);
    }
    ...
    public <T> boolean checkGreaterThan(String arg, T t1) {
        return Validation.NUMBER.matches(arg)
                && NumberUtil.checkPosition(ReflectionUtils.safeCast(arg), t1, true, false);
    }
    ...
    public <T> boolean checkGreaterThanEquals(String arg, T t1) {
        return Validation.NUMBER.matches(arg)
                && NumberUtil.checkPosition(ReflectionUtils.safeCast(arg), t1, true, true);
    }
    ...
    public <T> boolean checkLowerThan(String arg, T t1) {
        return Validation.NUMBER.matches(arg)
                && NumberUtil.checkPosition(ReflectionUtils.safeCast(arg), t1, false, false);
    }
    ...
    public <T> boolean checkLowerThanEquals(String arg, T t1) {
        return Validation.NUMBER.matches(arg)
                && NumberUtil.checkPosition(ReflectionUtils.safeCast(arg), t1, false, true);
    }
    ...
    public <T> boolean checkEquality(String arg, List<T> ts) {
        T o = (T) ReflectionUtils.safeCast(arg);
