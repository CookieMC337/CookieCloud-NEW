package de.cookiemc.driver.reaction;
...
public class Reaction {
     ...
     */
    public static boolean react(boolean condition, Runnable procedure, Integer... flags) {
        if(condition) {
            List<Integer> flagList = Arrays.asList(flags);

            try {
                if(flagList.contains(ASYNC)) {
                    executors.execute(procedure::run);
                }
                else {
                    procedure.run();
                }
    ...
    public static <T> boolean react(T object, Reactor<T>... reactors) {
        ...
        for(Reactor reactor : reactors) {
            cancelled = !Reaction.react(reactor.getObject(), object, reactor::invoke) || reactor.isCancelled();
        }
        return cancelled;
    }
     ...
     * @return Exception or not
     */
    public static boolean react(Object o1, Object o2, Runnable procedure, Integer... flags) {
        List<Integer> flagList = Arrays.asList(flags);
